...
type PersistedTranscriptScan struct {
	...
	totalEntries             int
	recentTail               TranscriptWindowSnapshot
...
func (s *PersistedTranscriptScan) RecentTailSnapshot() TranscriptWindowSnapshot {
	...
	return TranscriptWindowSnapshot{
		Snapshot:     ChatSnapshot{Entries: clonePersistedChatEntries(s.recentTail.Snapshot.Entries)},
		TotalEntries: s.recentTail.TotalEntries,
		Offset:       s.recentTail.Offset,
	}
}
...
func (s *PersistedTranscriptScan) materialize() {
	...
	if s.request.TrackRecentTail && s.request.TailLimit > 0 {
		tail := s.projector.RecentTailSnapshot(s.request.TailLimit)
		s.recentTail = TranscriptWindowSnapshot{
