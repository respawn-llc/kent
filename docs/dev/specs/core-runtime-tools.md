...
- A headless Session must persist its selected role identity. Resuming without a role selector must reapply the recorded role, including a role that is no longer available in the catalog. Existing Sessions must retain their recorded role selection. Opening such a Session in TUI or Desktop must preserve that identity. When no role was recorded, a headless continuation must preserve the Session's existing settings while applying the `default` role's delegation policy. After the Session Contract is locked, a later role selection must not replace the retained role.
