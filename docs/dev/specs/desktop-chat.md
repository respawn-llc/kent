...
- Expanded flat rows use the transcript width without a nested island. They are borderless and neutral except for the semantic color of their leading status icon. The leading icon represents the row type. The header uses the existing typed compact label or preview as its text and adds no separate textual type label. When no compact text is supplied, Desktop derives the same typed presentation as the TUI. The header also shows available status or actions and toggles expansion without taking row-specific actions.
...
- Supervisor has exactly `Off`, `After edits`, and `Always`, with descriptions `No automatic review`, `Review turns that changed files`, and `Review every completed turn`. The selected Agent's authoritative Supervisor baseline is part of Settings, and Desktop highlights that one choice as the default.
- Thinking follows the server-provided ordered values. Enumerated values use a stepped control and display the exact chosen value; Low is gray, Medium and High use the primary tone, and Xhigh, Max, and Ultra use the secondary tone. Unsupported Thinking is omitted. A supported non-enumerated value uses an input and Apply action. A mutation that is unavailable in the request's independently loaded current capability projection is rejected; rejected input remains entered and shows notification feedback.
- Fast mode is omitted when unsupported, and an unavailable Fast mutation is rejected. Questions remains visible and editable as a persisted on/off policy when the selected Agent lacks `ask_question`; the unavailable capability still means that Agent cannot call the tool. Required Auto-compaction remains on and unavailable. When compaction is disabled by Session policy, Auto-compaction shows its stored value but is unavailable. Required and disabled policy reject Auto-compaction mutations and preserve the stored value.
- Existing Sessions use one exhaustive semantic settings mutation operation. Each request selects one Agent, Supervisor, Thinking, Fast, Questions, or Auto-compaction action. An accepted request reports whether durable state changed and returns complete available Settings and Context; a typed current-state rejection returns the same complete available projections. Malformed requests, inaccessible or unknown Sessions, authentication failures, persistence failures, and server faults remain ordinary operation errors.
...
- New Chat reads stateless typed settings choices and current defaults for its selected Project workspace. This read creates and mutates no server Chat state.
- New Chat setting changes remain transient in that mounted presentation and are discarded when it is disposed. There is no pre-Session settings mutation operation.
- A New Chat mutation sends the complete Agent and settings aggregate displayed by the mounted New Chat presentation. The server validates it against the newest authoritative choices and applies it before returning the mutation result. If an Agent or enumerated choice became unavailable after the defaults read, the server replaces that choice with the newest applicable baseline instead of rejecting the mutation because of that configuration race.
- Desktop may retain one process-local cache of New Chat settings choices and defaults. Every Chat open and every owning configuration change invalidates and refetches that cache.
...
- Session Name and non-Agent settings remain available while work runs. Their mutations follow the [Runtime Steering And Model Loop](runtime-steering-loop.md) Session-setting contract.
...
- Existing Session drafts remain server-owned and persist through ordinary Session lifecycle behavior, including workspace detachment.
...
- A pending Goal response replaces the requested presentation with the server's identity-less preview. An acceptance-only response retains the requested presentation while that destination remains mounted.
...
- After pending or acceptance-only settlement leaves accepted presentation, the next authoritative Goal-observation fact admitted by that destination ends it regardless of values. Desktop does not use Goal identity, payload equality, a timeout, or mutation correlation as application confirmation.
- A later queued-application failure without another admitted authoritative Goal-observation fact does not fabricate Goal state or by itself remove accepted presentation while its destination remains open. The next authoritative Goal-observation fact ends accepted presentation and displays that fact.
