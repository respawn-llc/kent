...
- Reopening Goal for the same Session creates a new destination source and starts a new stale-tolerant sequence-1 hydration. It inherits no prior destination presentation, optimistic state, mutation-busy state, or callbacks. Desktop opening does not call Goal Show or gate on a separate Goal read.
...
- While a destination mutation is unresolved, that destination presents the requested Goal state. Authoritative Goal-observation facts admitted during the request still become its underlying authority but do not replace unresolved presentation before settlement.
- An authoritative Goal or Clear response replaces the destination authority only when that destination admitted no newer Goal-observation fact after the request began. A newer admitted fact always wins, including when it contains values equal to an earlier fact.
- When no newer Goal-observation fact was admitted, the mutation response also replaces Goal availability with its supplied value or clears availability when the response omits it. When a newer fact was admitted, that fact's availability wins.
- If the destination admits any authoritative Goal-observation fact after the current mutation began, that fact wins when the request settles. Consecutive server-accepted mutations may therefore show an intermediate earlier mutation state after the later request settles and before the later mutation's own Goal observation arrives.
- After any successful mutation response, the destination ends unresolved presentation and displays its winning authoritative Goal fact. Desktop retains no accepted or provisional response state.
- Closing a destination disposes its mutation guard and detaches every callback from its source. A reopened destination may start another mutation while the earlier RPC still settles; server authority orders accepted work. The earlier response can update neither the reopened destination nor the Chat affordance.
- Every Goal mutation failure uses Sonner even if the initiating Goal destination has closed. If that destination remains mounted, Desktop removes its mutation presentation, displays the latest authoritative Goal state without rolling it back, and restores the captured draft, edit, and focus state. Save failure preserves the complete dirty objective draft. A result delivered after disposal has no destination state callback or restoration, but its operation completion still reports the failure. A committed Goal Set applies its Session and authoritative Goal before showing exactly one warning notification for its returned diagnostic, including after the sidebar closes; the warning does not change the successful result and creates no success notification. Successful mutations trigger no additional Goal read, polling, or replay.
...
- After Goal observation reconnect, the mounted destination retains its last authoritative state, destination-local presentation, and draft until replacement stale-tolerant hydration arrives. It does not depend on transcript recovery and does not show an empty state during replacement.
...
- A completely validated New Chat Goal Set success delivers the resolved Session and authoritative Goal together. The matching Chat destination applies the Goal before any returned diagnostic warning.
- If Goal validation or admission then fails, the newly created Session remains. Desktop keeps the complete dirty Goal draft in the open sidebar, starts no Goal work for the rejected request, and uses ordinary Goal mutation error feedback.
