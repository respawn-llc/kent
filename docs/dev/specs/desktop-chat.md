...
- If Goal observation fails, the mounted destination must retain its last authoritative state, destination-local presentation, and draft and show Error with Retry. Retry must start one independent observation without depending on transcript observation.
