...
- Expanded Web Search must use simple text and existing presentation elements. It must show supplied search queries as small list items, followed by results, with sources last. Each list must preserve provider order. Sources must remain visible even when their URLs repeat result URLs.
