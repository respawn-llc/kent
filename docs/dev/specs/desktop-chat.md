...
- Each non-conversational item has a compact and full presentation except for Web Search, whose query row is not expandable. Expansion is local to the visible row and returns to that row type's default after the row leaves and re-enters the viewport. Expanded rows alone show their actions.
...
- Desktop does not migrate or reconstruct successful Question answers from event-log v1 Sessions, which do not retain typed answer facts. Encountering one follows the transcript contract-failure path; Desktop neither omits the row nor parses its human-readable tool result.
- At the transcript response boundary, Desktop treats only three Ask Question semantic violations as contract failures: a successful Question without typed answer facts, a recommendation outside zero or the one-based suggestion range, and a canceled or interrupted Question without nonblank error detail. Question and suggestion content, answer shape, selected-answer consistency, and answer facts on other tool states remain server producer invariants that Desktop does not independently revalidate.
