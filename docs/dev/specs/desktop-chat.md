...
- Successful compaction replaces the bounded transcript once through authoritative hydration. A failed recovery preserves committed content and the operator's loaded-history position.
...
- Existing-Session Settings mutations become available as soon as the global connection is connected, including while the reissued Settings read remains pending. Settings reads and mutations apply complete results as delivered without a freshness barrier, so a later-delivered read may temporarily replace a newer mutation result.
...
- Scratch Rehydration uses the ordinary sequenced transcript hydration and independent owner reads. Recovery preserves the committed transcript window and its currently loaded older/newer paging boundaries until replacement hydration arrives, and it never replaces visible rows with Loading. New deliberate paging remains available against those retained opaque cursors while observation waits. Replacement hydration carries the authoritative global newest segment, whether older history exists, and its opaque older cursor exactly when it does; it carries no newer cursor and atomically replaces the retained window and boundaries. Scratch Rehydration adds no automatic newest-page request, client transcript repair, duplicate suppression, history rewrite, freshness fence, or global snapshot/replay mechanism.
- Replacement transcript hydration admits any Goal fact only to the current Chat projection under its ordinary newer-fact rule; it does not update an open Goal destination. Replacement hydration that reports no Context usage displays zero usage values until a later update supplies current usage. Only omitted execution-target details preserve their prior displayed value.
- A continuity or contract failure starts at most one automatic replacement observation. That observation may wait for a dormant Session without disabling retained transcript paging or showing Loading. If it fails before valid hydration, the transcript settles in Error with Retry and starts no further automatic replacement. Each Retry starts one replacement attempt.
...
- Waiting for an Active Session Runtime has no spinner, notice, or error presentation. Runtime start begins hydration and live updates silently. Normal Runtime stop returns observation to waiting silently. Control-connection loss uses the global connection warning. Isolated transcript-socket loss retries silently and adds no warning.
