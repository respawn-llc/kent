...
- Every Goal mutation failure uses Sonner even if the initiating Goal destination has closed. If that destination remains mounted, Desktop removes its mutation presentation, displays the latest authoritative Goal state without rolling it back, and restores the captured draft, edit, and focus state. Save failure preserves the complete dirty objective draft. A result delivered after disposal has no destination state callback or restoration, but its operation completion still reports the failure. Successful mutations show no success notification and trigger no additional Goal read, polling, or replay.
