...
- Selection-required results must distinguish Workflow-required selection, an unavailable configured target, and an unavailable original target when reopening a completed Task. Every selection flow offers all four concrete modes.
...
- Conservative repair never recreates a missing branch from the old base commit, overwrites an existing directory, resets or renames a branch, accepts detached HEAD, repairs another repository, or infers ownership by scanning arbitrary roots. Unsafe or ambiguous states return one typed locked-target error with a small product-level cause, except when completed-Task reopening requires replacement selection as defined below.
- When a completed Task is manually moved into executable work, Kent must reuse its valid Execution Target or conservatively restore it when possible. If the original target is unavailable or unsafe to reuse, Kent must require an explicit replacement selection before applying the Move.
- Desktop and CLI must explain why the original target cannot be reused and offer no managed Worktree, source `HEAD`, repository default branch, and custom Git ref. Selection and cancellation must leave the Task Done with its content unchanged.
- Kent must prepare the replacement Execution Target and complete required setup before applying the Move and saving the replacement target. Target-resolution or setup failure must leave the Move unapplied and surface diagnostics.
...
- Moves between Terminal Nodes must not require Execution Target selection. Unfinished Tasks and Resume retain locked-target rules without replacement.
- Outside completed-Task reopening and the Compatibility Data case where a managed Worktree root equals its source Workspace root, a locked Execution Target must not be replaced or converted to no managed Worktree.
...
- After creation, an initiating request may repeat the exact branch recorded in managed Worktree metadata as an idempotent assertion. Outside the completed-Task replacement flow, a different branch is rejected as an attempted rename. This assertion also applies when an overlapping request supplied a post-snapshot replacement before the Worktree bound and encounters that Worktree afterward.
