...
- GUI workflow join nodes are internal arrow-merge plumbing, not user-visible workflow states. Kanban board read models must omit join columns and join group memberships. Workflow editor rendering shows join nodes as small inspectable merge diamonds so operators can inspect the real graph plumbing; joins still do not become board columns or task states.
- Workflow editor graph color semantics: edge color communicates context-preservation mode (`new_session` primary/blue, `continue_session` neutral/gray, `compact_and_continue_session` secondary/yellow); node color communicates node kind (`start` primary/blue, `agent` neutral/gray, `join` secondary/yellow, `terminal` success/green). Error/red is reserved only for validation errors and overrides normal semantic coloring when a graph entity is invalid.
...
- GUI MVP approval UI shows the stored approval snapshot: source node, transition label/id, target node(s), derived required provision fields/values, commentary, workflow version, and stale warning if relevant.
...
- Workflow definitions may be saved, linked, and made a project default while graph/project-context validation fails so users can iterate on drafts through GUI/API. Task creation, task start, and runtime scheduling require accumulated project-context validation and must reject with all safe actionable errors instead of creating invalid work.
- Workflow automation starts through an explicit task-start operation, exposed as `builder task start <short-id>` for the internal CLI and as a service method. It applies the start node's single-edge outgoing transition group and requests automation for the first executable placement. Arbitrary manual moves remain later approval/manual-move behavior.
- A workflow that is valid for task creation/automation has exactly one start node and, in the task-start flow, exactly one outgoing transition group containing exactly one edge to an agent node.
...
- Workflow runtime config lives under `[workflow]`: `completion_mode = "auto"`, `concurrency = 5`, `max_final_answer_violations = 3`, and `max_invalid_completion_attempts = 5`. Invalid values fail config validation before workflow workers start.
- Workflow required inputs are modeled on consuming agent nodes as named top-level string fields with user-authored descriptions. Prompt `.Inputs.*` placeholders validate against the consuming node's required inputs, not edge input bindings. The first executable node reached from `start` cannot declare upstream inputs and should use task fields such as `.TaskTitle` and `.TaskBody`.
- User-authored source-node output fields, edge input bindings, and edge output requirements are not canonical workflow-editing concepts. The server derives provision fields, same-name input bindings, selected-transition output requirements, and possible completion fields from node required inputs, graph topology, and join provider selections.
- Workflow completion schemas expose derived possible provision fields as optional/nullable named top-level string fields. Runtime validates the selected transition group against the frozen derived requirements and rejects missing required provision values, unknown provision values, and non-string or trimmed-empty required values. Workflow schemas and runtime output values have fixed pragmatic size limits so a bad model response cannot create unbounded DB/provider payloads: field name `<= 64` chars, field description `<= 1000` chars, output value `<= 64 KiB`, commentary `<= 64 KiB`, and task comment body `<= 256 KiB`.
- Workflow run-start and transition snapshots are derived-only runtime contracts. Runtime must use the frozen snapshot for in-flight work and must not recompute missing wiring from the latest workflow graph as a fallback.
- Join provider selection is keyed by downstream required input and points at the actual incoming edge into the join. Joins copy the selected provider branch's same-named field value for each downstream input; there is no generic join aggregate mode. Missing providers, unavailable provider fields, and incompatible same-name provider overlaps are hard validation errors.
...
- Workflow persistence must not store durable run states for pending scheduler work or active runtime ownership. Runnable and active-execution conditions are derived from durable automation intent plus live runtime/scheduler state. User cancellation and runtime failures collapse into interrupted outcomes with reason metadata.
- Workflow runtime must enforce mandatory cheap protocol caps: normal final-answer violations and invalid workflow-completion attempts stop the run as interrupted with actionable reason metadata after `[workflow].max_final_answer_violations = 3` or `[workflow].max_invalid_completion_attempts = 5`. No wall-clock cap is required for v1.
- All executable agent nodes require and reuse the task managed worktree. Builder creates the managed worktree on task start before the first executable run is scheduled.
- Workflows carry a monotonic `version` that increments on persisted definition changes, including metadata edits and graph-affecting node/group/edge edits. Tasks, runs, transitions, and approval/edge snapshots store the version they observed for traceability and stale-warning UX. This is not full graph versioning: run-start snapshots and transition/approval/fan-out edge snapshots keep using their snapshot, and anything not snapshotted uses the current workflow graph/config at execution time.
...
- Project workflow links are active membership rows only. Do not soft-unlink project-workflow links. Unlink hard-deletes an unused link; if tasks exist for that link, the user must move/delete those tasks before unlinking. Blocked workflow unlink returns typed blockers with task counts and task references instead of an opaque error. Retiring a workflow means deleting the workflow definition and cascading/deleting its tasks through an explicit workflow deletion path.
- Workflow tasks should normalize project-workflow pairing around `project_workflow_links`: `tasks.project_workflow_link_id` is the single source of truth for the task's project/workflow pair, while task project/workflow read-model fields are derived through the link. Direct duplicated `tasks.project_id` and `tasks.workflow_id` columns should be removed before new workflow GUI work unless a hard independently-needed invariant is documented. This normalization is a hard cutover with no compatibility shim around the old duplicated task columns.
- Project-owned default pointers for the workflow schema cutover use `projects.default_project_workflow_link_id` and `projects.primary_workspace_id`; enforce that each pointer references a row owned by the same project.
- Workspace/session schema cutover migration policy: deterministically backfill `projects.primary_workspace_id` from the oldest workspace when no single primary workspace row exists, but fail fast on true relationship contradictions such as session workspace outside its project, session worktree outside its workspace, or managed task worktree outside the task's source workspace/project.
- Workspace/worktree display labels, availability, primary/default status, and main-worktree status are read-model facts, not stored metadata columns. Workspace labels and availability are derived from canonical root paths, primary/default workspace status is owned by `projects.primary_workspace_id`, and worktree main status is derived from canonical worktree root matching its workspace root while Git metadata remains stored for Git/worktree behavior.
- Task comments are hard-deleted task-local notes. Comment rows do not store source-run links, deleted tombstones, or opaque metadata; include-deleted comment APIs and read-model state are not part of the product.
- Runtime leases are durable controller-token facts only. SQLite stores `id`, `session_id`, and `created_at_unix_ms`; request/client/acquired/metadata correlation remains in request DTOs, process memory, or logs, not persisted lease rows.
- Workflow invalidation events are process-local live signals, not durable/replayable sequence state. SQLite does not store `workflow_events`; workflow project subscriptions have no `after_sequence` cursor and event payloads have no durable `sequence`. GUI clients must refetch read models after subscription ACK/reconnect/error and treat live events as invalidation hints only.
- There is no product archive lifecycle for workflows or workflow nodes. Remove the hidden `ArchiveNode`/`workflow_nodes.metadata_json.$.archived_at_unix_ms` path during schema cleanup rather than preserving or typing archive metadata.
- Workflow definitions do not persist opaque metadata. SQLite does not store `metadata_json` on `workflows`, `workflow_nodes`, `workflow_node_groups`, `workflow_transition_groups`, or `workflow_edges`; public workflow-node-group APIs also do not expose an opaque metadata field. Workflow graph storage derives transition-group and edge workflow membership from `workflow_transition_groups.source_node_id -> workflow_nodes.workflow_id`, and enforces edge target nodes to stay in the source-node workflow instead of storing `workflow_transition_groups.workflow_id` or `workflow_edges.workflow_id`.
- Workflow deletion impact previews return counts only. Confirmed workflow deletion removes workflow-linked task database rows and workflow links/graph rows, blocks when affected tasks have active or runnable workflow runs, and blocks if deleting a default workflow would leave a project with other workflow links but no replacement default. By default deletion is DB-only and preserves artifacts/worktrees; artifact/worktree cleanup is an explicit opt-in path for future UI, not the default destructive operation.
- Batch workflow graph save uses a store-owned transaction shape with expected `version`, draft validation, process-local edit semantics, typed node/edge task-reference blockers, and confirmation for unreferenced graph row removals. Graph saves never delete or move tasks; whole-workflow deletion is the task-deleting path.
- The first visible workflow editor editing slice combines GUI route-local draft state with workflow metadata editing and agent-node editing. It does not split draft plumbing and first visible forms into separate implementation slices.
- Workflow editor drafts are route-local GUI state, but draft-backed global sidebar forms cross the `AppChrome` sibling boundary through a shell-level bridge registered by the editor route. The route remains the lifecycle owner; the bridge is not a server draft, React Query cache, or durable state owner.
- Workflow graph save/preview/validate draft contracts may carry optional workflow metadata for the editor Save flow. This intentionally revises the earlier graph-only DTO constraint so one route-level Save can apply metadata and graph edits atomically.
- Workflow definitions have a single monotonic `version` over persisted definition changes. Metadata-only changes and graph changes each increment it once; combined metadata+graph saves also increment it once; no-op saves increment neither.
- Workflow editor Save computes normalized `metadataChanged` and `graphChanged` server-side. Optimistic version checks run for any actual metadata or graph change. Graph edit policy, active-work blockers, removal impact, and confirmation run only when `graphChanged`; metadata-only and no-op saves bypass graph edit policy.
- Workflow-scoped subscriptions/events are required for Workflow Library editor mode so global workflow editing has the same reactive conflict behavior as project-linked editing.
- Workflow editor draft validation and execution validation are shown separately. Graph-dirty saves are gated by draft blocking validity and save-preview blockers. Metadata-only saves may proceed even when the existing graph has draft validation errors; execution validation errors remain visible but do not block saving.
- Approved workflow-schema minimization for the next schema cutover includes removing redundant indexes `runtime_leases_session_idx`, `workspaces_project_idx`, `workflow_transition_groups_source_transition_idx`, and `tasks_project_short_id_idx` when equivalent unique/leading-key indexes remain; removing `runtime_leases.request_id`; deriving workspace/worktree labels instead of storing `workspaces.display_name` and `worktrees.display_name`; removing `task_comments.source_run_id`; replacing comment soft-delete with hard delete; and removing transition display duplicates from `task_transitions` once run-start snapshots are authoritative. Keep `tasks.source_url` as a structured task field. Do not remove `sessions.first_prompt_preview`, `sessions.input_draft`, `task_comments.author_id`, or `task_transition_edges` display/config snapshots without a later product decision.
...
- Fan-out/join uses static graph derivation in v1. A fan-out transition group must have exactly one unambiguous nearest common join reachable from every branch; branch paths before that join may not terminate, enter nested fan-out, or contain cycles. Ambiguous/complex fan-out topology is rejected by validation.
...
- Users manage goals primarily through TUI `/goal`; models may use normal shell commands `builder goal show`, `builder goal complete`, and `builder goal set <objective>` for the current session, relying on Builder-injected caller session identity for correct session targeting. Agent `builder goal set` is only allowed when no active or paused goal exists; a completed goal is terminal history and does not block the next agent-set goal. Agent attempts to overwrite an active or paused goal, pause, resume, or clear must fail nonzero with model-facing warning copy from `prompts/goal/agent_command_denied.md`.
