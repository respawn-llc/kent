...
- Workflow package boundaries are locked for implementation: `server/workflow` is pure domain types, validation, and state-machine logic; `server/workflowstore` adapts metadata persistence; `server/workflowsvc` owns use-case/service orchestration; `server/workflowscheduler` owns runnable derivation and workers; `server/workflowruntime` adapts sessions/runtime/headless infrastructure; `server/workflowview` owns read models.
