...
- GUI workflow join nodes are internal arrow-merge plumbing, not user-visible workflow states. Kanban board read models must omit join columns and join group memberships, and workflow editor rendering must collapse `source -> join -> target` into direct visual merge edges while preserving runtime graph semantics.
