...
- GUI MVP approval UI shows the stored approval snapshot: source node, transition label/id, target node(s), required output fields/values, commentary, workflow version, and stale warning if relevant.
...
- Edge-required node output fields require present, string-typed, trimmed non-empty values. Optional fields may be omitted or empty. Workflow schemas and runtime output values have fixed pragmatic size limits so a bad model response cannot create unbounded DB/provider payloads: output field name `<= 64` chars, output field description `<= 1000` chars, output value `<= 64 KiB`, commentary `<= 64 KiB`, and task comment body `<= 256 KiB`.
