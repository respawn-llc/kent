...
- Workflows carry a monotonic `graph_revision` that increments on graph-affecting node/group/edge edits. Tasks, runs, transitions, and approval/edge snapshots store the revision they observed for traceability and stale-warning UX. This is not full graph versioning: run-start snapshots and transition/approval/fan-out edge snapshots keep using their snapshot, and anything not snapshotted uses the current workflow graph/config at execution time.
...
- Batch workflow graph save uses a store-owned transaction shape with expected `graph_revision`, draft validation, process-local edit semantics, typed node/edge task-reference blockers, and confirmation for unreferenced graph row removals. Graph saves never delete or move tasks; whole-workflow deletion is the task-deleting path.
...
- Workflow definitions have a monotonic `definition_revision` separate from `graph_revision`. Metadata changes increment only `definition_revision`; graph changes increment both; no-op saves increment neither.
- Workflow editor Save computes normalized `metadataChanged` and `graphChanged` server-side. Graph edit policy, active-work blockers, removal impact, confirmation, graph revision checks, and graph revision increments run only when `graphChanged`; metadata-only and no-op saves bypass graph edit policy.
