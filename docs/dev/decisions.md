...
- Workflows carry a monotonic `version` that increments on graph-affecting node/group/edge edits. Tasks, runs, transitions, and approval/edge snapshots store the revision they observed for traceability and stale-warning UX. This is not full graph versioning: run-start snapshots and transition/approval/fan-out edge snapshots keep using their snapshot, and anything not snapshotted uses the current workflow graph/config at execution time.
