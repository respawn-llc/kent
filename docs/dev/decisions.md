...
- Workflow definitions may be saved, linked, and made a project default while graph/project-context validation fails so users can iterate on drafts through GUI/API. Task creation, task automation start/schedule, and runtime scheduling require accumulated project-context validation and must reject with all safe actionable errors instead of creating invalid work.
