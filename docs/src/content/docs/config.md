...
**Connection discovery is an exception.** Goal, binding, question and worktree CLI commands discover the server endpoint from existing global/shared configuration and `KENT_SERVER_HOST`/`KENT_SERVER_PORT`, without opening the server's metadata database locally. The running server resolves Main Workspace private ownership and operational settings after connection. A private-only endpoint override does not control pre-connection discovery from a managed worktree; put that endpoint in global/shared configuration or the environment.
...
- Private settings live at `<main-workspace-root>/.kent/config.local.toml`. The file is optional and accepts workspace-allowed settings. Keep personal overrides uncommitted using your own Git ignore configuration; Kent does not change Git rules or copy this file into worktrees.
...
| `system_prompt_file`                  | optional string | unset         |                                            |                                    | Selected custom main prompt file. Relative paths resolve from the supplying configuration file. Higher layers replace the selection; an empty path is invalid.                                                                                                                                                                                                                                                                                                                                                                                                                       |
...
| `server_host`                         | string          | `127.0.0.1`   | `KENT_SERVER_HOST`                         |                                    | Exact TCP app-server host to dial or listen on. Goal, binding, question and worktree commands discover it from global/shared settings or the environment, excluding private settings. Kent does not use discovery files or silent port rebinding. Same-machine Unix socket optimization does not override an explicit TCP target.                                                                                                                                                                                                                                                    |
| `server_port`                         | int             | `53082`       | `KENT_SERVER_PORT`                         |                                    | Exact TCP app-server port to dial or listen on. Goal, binding, question and worktree commands discover it from global/shared settings or the environment, excluding private settings. Must match across clients attached to the same persistence root. Same-machine Unix socket optimization does not override an explicit TCP target.                                                                                                                                                                                                                                               |
