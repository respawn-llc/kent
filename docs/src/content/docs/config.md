...
| `prevent_sleep` | string | `active` | `BUILDER_PREVENT_SLEEP` |  | Prevent system sleep while Builder is running. Allowed: `always` (while the server process is live), `active` (while any agent is working, plus up to one minute of idle-confirmation grace), `never` (disabled). Only system sleep is inhibited; screensaver and display sleep are unaffected. |
