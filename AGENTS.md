...
- Full transcript history is unbounded & weighs gigabytes, thus no code must ever attempt to load `events.jsonl` fully into memory. Project through streaming/windowed readers (`Store.WalkEvents`, `Store.ReadRuns`, the persisted transcript scan); fork/clone stream the parent log into the child in bounded chunks. Full-history collectors are test-only and live in `server/session/sessiontest` (never importable by production). A repo-wide guard (`server/core/event_log_materialization_test.go`) fails the build if production code calls a full-history materializer or imports the test-support package.
- User-visible transcript is served from disk, never from a resident copy. Both active and dormant transcript reads project the persisted event log through `streamingTranscriptScan` (wrapped by `PersistedTranscriptScan`), which feeds the window-bounded `inMemoryTranscriptScan` and retains only the requested page/recent-tail window; the engine's `RecentTailTranscriptWindow`/`TranscriptPageSnapshot`/`ChatSnapshot` scan `e.store` and overlay only the live streaming delta. The in-memory `chatStore` holds the bounded model working set, not the transcript: `replaceHistory` trims pre-cutoff provider items + local entries and prunes tool completions to the working set, leaving an `O(1)` committed-entry counter for hot-path delta detection. `RecentTailTranscriptWindow` carries a compaction-boundary floor (current active context); reminder/transcript dedupe that must see pre-compaction entries uses the floorless `recentTranscriptEntries` page instead. Do not reintroduce in-memory full-transcript readers or a resident role-2 transcript buffer.
