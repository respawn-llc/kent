...
- Home's native Project-created observation uses a bounded buffer. Overflow fails the observation through the ordinary temporary status notification without restart or replay. `Queue.failCauseUnsafe` with a typed failure is permitted on that supplied native Stream queue only for overflow.
