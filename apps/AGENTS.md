...
- The workflow editor subscription adapter may use `Queue.failCauseUnsafe` only on the queue supplied by its `Stream.callback` to deliver terminal errors and completion independently of its bounded refresh buffer.
