...
- Shell observation failures follow these presentation rules: focus registration and initial-read failures log diagnostics and publish unfocused state. File-drop and Project-deletion registration failures log diagnostics. Native notification activation registration failures log diagnostics without an additional status notification. Attention observation failures show a temporary notification with explicit Retry for that observation.
- Native integration may use private platform callback storage. Application-facing observation contracts expose Streams or readonly Atoms. The Effect policy checks those exported contracts even inside native integration.
