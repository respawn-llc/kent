...
export async function fakeDescriptorResult(route: FakeDescriptorRoute, request: Message, callIndex: number) {
  const result = await (route.resultFactory?.(request, callIndex) ?? route.result);
  if (result === undefined)
    throw new Error(`Missing fake descriptor result: ${operationName(route.descriptor)}`);
  return result;
}

export function worktreeBrowserFixtureEntry(
  kind: "mainWorkspace" | "registered" | "missing" | "external",
  current: boolean,
  ...
  const git = {
    ...
    headObject: "abc123",
    branchName: options.detached === true ? undefined : "feature",
  ...
  const selector = kind === "mainWorkspace" ? "main" : kind === "external" ? git.canonicalRoot : "feature";
  return create(ListEntrySchema, {
    topology: {
      topology:
        kind === "mainWorkspace" || kind === "external"
          ? { case: kind, value: { git } }
          : kind === "registered"
    ...
    projection: {
      selector,
      fallbackIdentity: kind === "external" && options.detached === true ? "feature" : undefined,
      isCurrent: current,
      ...fixtureOperations(
        kind,
        current,
        selector,
        kind === "external" ? git.canonicalRoot : kent.worktreeId,
      ),
    },
  });
}
...
function fixtureOperations(
  kind: Parameters<typeof worktreeBrowserFixtureEntry>[0],
  current: boolean,
  selector: string,
      ...
      ? {
          switch:
            kind === "mainWorkspace"
              ...
              : { kind: SwitchOperationKind.WORKTREE_SWITCH_OPERATION_ENTER, selector },
        }
      : {}),
    ...(kind !== "mainWorkspace" ? { deletePreview: { selector: deletionSelector } } : {}),
  };
}

export function worktreeBrowserFixtureRoute(worktrees: ListEntry[] = [], branchSuggestion?: string) {
  return {
...
export function worktreeQueryFixtureRoutes(
  options: Readonly<{
    detached?: boolean;
    createKind?: CreateTargetResolutionKind;
    cleanliness?: Readonly<{ kind: DirtyStateKind; dirtyFileCount?: number; unknownCause?: string }>;
  }> = {},
): readonly FakeRoute[] {
  const topology = {
    topology: {
          ...
          outcome: {
            ...
            value: {
              resolution: {
                ...
                input: decode(
                  CreateTargetService.method.resolve.input,
                  encode<DescMessage>(CreateTargetService.method.resolve.input, request),
                ).target,
                resolvedRef:
                  options.createKind ===
                  CreateTargetResolutionKind.WORKTREE_CREATE_TARGET_RESOLUTION_KIND_NEW_BRANCH
                    ? undefined
                    : `refs/heads/feature-${String(callIndex)}`,
              },
            },
          },
        }),
...
export class FakeRpcTransport implements DescriptorRpcTransport {
  ...
  async callDescriptor<Method extends DescMethod>(
    ...
    const result = await fakeDescriptorResult(route, request, callIndex);
