...
export function CollapsibleMarkdownViewport({
  ...
  const overflows = useMarkdownOverflow(collapsed, viewportRef, contentRef);
...
function useMarkdownOverflow(
  ...
  }, [collapsed, contentRef, viewportRef]);
  return overflows;
}

function MarkdownExpandAffordance({
  expandLabel,
  onExpand,
  phase,
}: Readonly<{
  ...
  if (phase === "hidden" || onExpand === undefined || expandLabel === undefined) return null;
