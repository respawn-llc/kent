...
fn show_attention_notification(
    ...
    close_existing_attention_notification_record(&active, backend_id)?;
...
fn close_existing_attention_notification_record(
    active: &Arc<Mutex<HashMap<u32, AttentionNotificationRecord>>>,
    backend_id: u32,
) -> Result<(), String> {
    let record = active
...
fn remove_attention_notification_backend(
    ...
    close_existing_attention_notification_record(&state.active, backend_id)
}
...
mod tests {
    use super::{
        clean_path, close_existing_attention_notification_record,
        insert_attention_notification_record, parse_theme, persistence_root_hash,
    ...
    #[test]
    fn replacing_attention_notification_closes_previous_surface_before_insert() {
        let state = AttentionNotificationState::default();
        ...
        close_existing_attention_notification_record(&state.active, 42).expect("close previous");
